...
MODEL_LSTM_PATH = 'sign_language_lstm.h5' # Model LSTM mới

CLASSES_STATIC = ['A', 'B', 'U', 'O', 'None'] 

CLASSES_LSTM = ['SAC', 'HUYEN', 'HOI', 'NGA', 'NANG', 'MU', 'MOC', 'TRANG']
...
print("✅ Hệ thống Hybrid sẵn sàng!")
...
sequence_buffer = [] # Bộ nhớ đệm 30 frame cho LSTM
...
while True:
    ...
    if results.multi_hand_landmarks:
        for hand_landmarks in results.multi_hand_landmarks:
            ...
            # Trích xuất Landmark tọa độ chuẩn (cho LSTM và model tĩnh)
            landmark_list = calc_landmark_list(debug_image, hand_landmarks)
            pre_processed = pre_process_landmark(landmark_list)
            
            # Đẩy vào bộ nhớ đệm cho LSTM
            sequence_buffer.append(pre_processed)
            sequence_buffer = sequence_buffer[-30:] # Giữ đúng 30 frame gần nhất

            frame_count += 1
            now = time.time()
            
            # --- 1. NHẬN DIỆN DẤU ĐỘNG (LSTM) - CHẠY LIÊN TỤC ---
            if len(sequence_buffer) == 30:
                res_lstm = model_lstm.predict(np.expand_dims(sequence_buffer, axis=0), verbose=0)[0]
                idx_lstm = np.argmax(res_lstm)
                conf_lstm = res_lstm[idx_lstm]
                label_lstm = CLASSES_LSTM[idx_lstm]
                
                if conf_lstm > 0.9 and (now - last_time_recognized) > DELAY_TIME:
                    if sentence:
                        pair = (sentence[-1], label_lstm)
                        if pair in VIETNAMESE_LOGIC:
                            sentence[-1] = VIETNAMESE_LOGIC[pair]
                            last_time_recognized = now
                            sequence_buffer = [] # Reset buffer sau khi nhận dấu
            
            # --- 2. NHẬN DIỆN CHỮ TĨNH (CNN + LANDMARK) - MỖI 5 FRAME ---
            if frame_count % SKIP_FRAMES == 0:
                ...
                if (sx2 > sx1) and (sy2 > sy1):
                    ...
                    scores_cnn = np.zeros(len(CLASSES_STATIC))
                ...
                current_label = CLASSES_STATIC[idx]
            ...
            # Vẽ box thông tin
            bx1, by1, bx2, by2, _ = get_bbox(landmark_list, h, w)
