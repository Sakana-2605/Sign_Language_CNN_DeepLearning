...
#     max_num_hands=1,
#     model_complexity=1, # 0: Nhanh, 1: Cân bằng 
...
#                 # 4. Vẽ UI
#                 label = signs[idx] if conf > CONF_THRESH else "Scanning..."
...
while cap.isOpened():
    ...
    if results.multi_hand_landmarks:
        for hand_lms in results.multi_hand_landmarks:
            ...
            prediction = model.predict(np.array([processed]), verbose=0)
            idx = np.argmax(prediction)
            conf = np.max(prediction)

            if conf > 0.8:
                label = f"{actions[idx]} ({conf*100:.1f}%)"
